...

{
  ...
  hd->SetLineWidth(2);
  t1->Draw("dspmass>>hd", "d0mass < 1.994 && d0mass > 1.706"); //5 sigmas window around d0 mass
  ...
  TLegend *leg1 = new TLegend(0.2, 0.45, 0.5, 0.65);
  leg1->SetFillColor(kWhite);
  leg1->SetLineColor(kWhite);
  leg1->SetTextSize(0.035);
  leg1->AddEntry(frame1->findObject("hd"),"5#sigma D^{0} Mass Range","l");
  ...
  leg1->AddEntry(frame1->findObject("hd3"),"Total","l");
  leg1->AddEntry(frame1->findObject("hd2"),"Truth Matched","l");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("/home/taylor/Research/plots/pizfitdspkstree/d0masswindowdspks.pdf","pdf"); 
  canvas->Print("/home/taylor/Research/plots/pizfitdspkstree/d0masswindowdspks.eps","eps");
