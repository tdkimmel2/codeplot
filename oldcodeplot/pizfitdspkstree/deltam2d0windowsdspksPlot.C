...
void deltam2d0windowsdspksPlot()

{
  
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/dspreconpizfit.root"); //put file here
  ...
  hd->SetLineWidth(2);
  t1->Draw("dspmass>>hd", "d0mass < 1.994 && d0mass > 1.706 && deltam2 > 0.5421 && deltam2 < 0.5861"); //5 sigmas window around d0 mass and 3 sigma around deltam2
  ...
  RooPlot* frame1 = dspmass.frame(Bins(nBins),Title("D*^{+} Mass With 5#sigma D^{0} Mass Window and 3#sigma #DeltaM^{2} Window From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  TLegend *leg1 = new TLegend(0.15, 0.7, 0.45, 0.9);//(x1, y1, x2, y2)
  leg1->SetFillColor(kWhite);
  leg1->SetLineColor(kWhite);
  leg1->SetTextSize(0.035);
  leg1->AddEntry(frame1->findObject("hd"),"5#sigma D^{0} Mass Window and 3#sigma #DeltaM^{2} Window","l");
  ...
  leg1->AddEntry(frame1->findObject("hd2"), "Truth Matched, No Cut", "l");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("/home/taylor/Research/plots/pizfitdspkstree/deltam2d0windowsdspkstruth.pdf","pdf"); 
  canvas->Print("/home/taylor/Research/plots/pizfitdspkstree/deltam2d0windowsdspkstruth.eps","eps");
