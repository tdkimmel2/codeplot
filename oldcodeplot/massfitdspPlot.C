...

{
  ...
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/dsptest.root"); //put file here
  TTree *t1=(TTree*) f1->Get("dsptree"); //put tree name here
  ...
  RooRealVar massfitchisq("massfitchisq", "massfitchisq", 0, 1000);
  ...
  RooPlot* frame1 = dspmass.frame(Bins(nBins),Title("D*^{+} Mass")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  //leg1->AddEntry(frame1->findObject("hd2"),"#Chi^{2} < 100","f");
  ...
  leg1->AddEntry(frame1->findObject("hd3"),"#Chi^{2} < 50","l");
  leg1->AddEntry(frame1->findObject("hd2"),"#Chi^{2} < 100","l");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("./massfit.pdf","pdf"); 
  canvas->Print("./massfit.eps","eps");
