...
void gausschebyd0ksfit()

{
 
  // Fit observables                                                                           
  RooRealVar d0mass("d0mass", "d0mass", 1.76, 1.96);
  //RooRealVar whoru("whoru", "whoru", 1, 2);

  //RooDataSet* data = new RooDataSet("data", "raw data", RooArgSet(d0mass, whoru));
  RooDataSet* data =  new RooDataSet("data", "raw data", RooArgSet(d0mass));
  ...
  for(int i=0; i < nevt; i++)
    {
      ...
      if(x_d0mass < 1.76 || x_d0mass > 1.96) continue;                                            
      //if(x_whoru == 1) continue;
                                                          
      d0mass.setVal(x_d0mass);
      //whoru.setVal(x_whoru);
   
      //data->add(RooArgSet(d0mass, whoru));                                                    
      data->add(RooArgSet(d0mass));
  ...
  RooRealVar sigmean1("<>1_{signal}", "<>_{signal}", 1.8659, 1.85, 1.87);
  //RooRealVar sigmean2("<>2_{signal}", "<>_{signal2}", 0.130, 0.135);
  RooRealVar sigwidth1("#sigma_{signal}", "#sigma_{signal}", 0.0249, 0.0, 0.04);
  ...
  RooGaussian gauss1("gauss1", "gaussian PDF", d0mass, sigmean1, sigwidth1);
  //RooGaussian gauss2("gauss2", "gaussian PDF", d0mass, sigmean1, sigwidth2);
  RooChebychev poly("poly", "poly", d0mass, RooArgList(c0, c1));
  ...
  RooPlot* yframe = d0mass.frame(45); 
  data->plotOn(yframe);
  pdf.plotOn(yframe);
  ...
  pdf.paramOn(yframe, Layout(0.16, 0.58, 0.65)); //Plots the pdf parameters on yframe with Layout(xmin, xmax, ymin) ymax is at the end of the last parameter line
  ...
  yframe->SetStats(0);
  yframe->GetXaxis()->SetTitle("D^{0} Mass (GeV/c^{2})");
  ...
  c->Print("/home/taylor/Research/plots/dspkstree/roofit/gausschebyd0ksfit.pdf","pdf");
  c->Print("/home/taylor/Research/plots/dspkstree/roofit/gausschebyd0ksfit.eps","eps");
  c->Print("/home/taylor/Research/plots/dspkstree/roofit/gausschebyd0ksfit.png","png");
