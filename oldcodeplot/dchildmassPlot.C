...

{
  ...
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/kstest.root"); //put file here
  ...
  */
  ...
  RooPlot* frame1 = dchildmass.frame(Bins(nBins),Title("D^{0} Mass From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("M_{D^{0}} (GeV/c^{2})");
  ...
  canvas->Print("./dchildmass.pdf","pdf"); 
  canvas->Print("./dchildmass.eps","eps");
