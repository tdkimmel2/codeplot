...

{
  ...
  TTree *t1=(TTree*) f1->Get("realdsptree"); //put tree name here
  ...
  RooPlot* frame1 = realdspmass.frame(Bins(nBins),Title("D*^{+} Mass")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  canvas->Print("./realdspmass.pdf","pdf"); 
  canvas->Print("./realdspmass.eps","eps");
