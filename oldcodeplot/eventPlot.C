...

{
  ...
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/k0Ltest.root"); //put file here
  TTree *t1=(TTree*) f1->Get("eventtree"); //put tree name here
  TTree *t2=(TTree*) f1->Get("gammatree");
  ...
  RooRealVar eventE("eventE", "eventE",0,20);
  RooRealVar gammaE("gammaE", "gammaE",0,10);

  //----------------------------------------------------------
  // This next bit calculates the bin width based on specified number of bins and the range of the RooRealVar defined above
  //----------------------------------------------------------
  double lb = eventE.getMin(); double rb=eventE.getMax();
  ...
  RooPlot* frame1 = eventE.frame(Bins(nBins),Title("Total Event Energy")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("Energy (GeV/c^{2})");
  ...
  //canvas->Print("./dptest.pdf","pdf"); 
  //canvas->Print("./dptest.eps","eps");
