...

{
  ...
  RooPlot* frame1 = ksmass.frame(Bins(nBins),Title("K^{0}_{S} Mass From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("M_{K^{0}_{S}} (GeV/c^{2})");
  ...
  //leg1->AddEntry(frame1->findObject("hd2"),"Signal","l");
  //frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("./ksrecon.pdf","pdf"); 
  canvas->Print("./ksrecon.eps","eps");
