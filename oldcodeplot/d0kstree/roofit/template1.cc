...

{
  ...
  // Fit observables                                                                           
  RooRealVar d0mass("d0mass", "d0mass", 1.835, 1.89);
  ...
  //// Read Data File
  TChain* chain_data = new TChain("d0tree");                                         
  chain_data->Add("/home/taylor/Research/root/ksrecon.root");                                                      
  ...
  for(int i=0; i<nevt;i++)
    {
      ...
      if(x_d0mass < 1.835 || x_d0mass > 1.89) continue;                                            
  ...
  RooGaussian gauss1("gauss1", "gaussian PDF", d0mass, sigmean, sigwidth1);
  ...
  RooFitResult* fitRes = gauss1.fitTo(*data);
