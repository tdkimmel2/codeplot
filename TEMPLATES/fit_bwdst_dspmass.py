...
dspmass = RooRealVar("dspmass", "dspmass", 2.003, 2.045)
...
vars = RooArgSet(dspmass)

data = RooDataSet("data", "raw data", t, vars)
#dChib2 = RooDataSet("dChib2","dChib2",t2,vars)
#dGeneric = RooDataSet("dGeneric","dGeneric",tgen,vars)
#dGen = RooDataSet("dGeneric","dGeneric",tgen,vars)

#dSig1 = dChib1.reduce("pi0tru==1")
#dSig2 = dChib2.reduce("pi0tru==1")
#dBkg1 = dChib1.reduce("pi0tru!=1")
#dBkg2 = dChib2.reduce("pi0tru!=1")
#dGeneric = dGen.reduce("dipiRecm>9.81||dipiRecm<9.78") 


#data = (RooDataSet*) rawData.reduce(cuts)
#data.setBins(nBins)
#dmchi = RooArgSet(deltamChi)
#dh_chib1 = RooDataHist("dh_chib1", "dh_chib1", vars, dChib1)
#dsphist = RooDataHist("dsphist", "dsphist", data)
#dh_chib2_sig = RooDataHist("dh_chib2_sig", "dh_chib2_sig", dmchi, dSig2)
#dh_chib1_bkg = RooDataHist("dh_chib1_bkg", "dh_chib1_bkg", dmchi, dBkg1)
#dh_chib2_bkg = RooDataHist("dh_chib2_bkg", "dh_chib2_bkg", dmchi, dBkg2)
#dh_gen_bkg = RooDataHist("dh_gen_bkg", "dh_gen_bkg", dmchi, dGeneric)

#print dh_chib1_sig
#chib1_sig_PDF = RooHistPdf("chib1_sig_PDF","chib1_sig_PDF",RooArgSet(deltamChi),dh_chib1_sig,9)

#dchib1Sig_1k = chib1_sig_PDF.generate(RooArgSet(deltamChi),43866)



voigmean = RooRealVar("<>_{signal}", "<>_{signal}", 2.01, 2.005, 2.015);
...
bwmean = RooRealVar("<>_{sig}", "<>_{sig}", 2.01, 2.005, 2.015);
bwwidth = RooRealVar("#sigma_{sig}", "#sigma_{sig}", 0.0005, 0, 0.01);

dm0 = RooRealVar("dm0", "dm0", 2.004, 2.00, 2.008);
d = RooRealVar("d", "d", 0.0114, 0, 2);
a = RooRealVar("a", "a", 2.5, 0, 10);
b = RooRealVar("b", "b", 28, 0, 50);

nsig = RooRealVar("N_{Signal}","nsig",0,100000)
nbkg = RooRealVar("N_{Bkg}","nbkg",0,100000)

bkg = RooDstD0BG("bkg","DstD0BG Bkg Fcn",dspmass,dm0,d,a,b)
#sig = RooVoigtian("sig","Voigtian Signal Fcn",dspmass,voigmean,voigwidth,voigsigma)
sig = RooBreitWigner("sig","Breit Wigner Signal Fcn", dspmass,bwmean,bwwidth)
...
#nSignal=t1.Draw("dspmass>>h1")
#print("%.0f signal events plotted in the histogram"%nSignal)

frame1 = dspmass.frame(RooFit.Bins(nBins),RooFit.Title("D^{*+} -> D^{0}(-> #pi^{0} + K_{S}^{0}) + #pi^{+}: From MC")) 
pullFrame = dspmass.frame(RooFit.Bins(nBins),RooFit.Title(""))
...
pullFrame.GetXaxis().SetTitle("D^{*+} Mass (GeV/c^{2})")
...
canvas.Print("/home/taylor/Research/plots/dtokpi/dspmass_breitwigner+dstd0bg_fit.pdf")
canvas.Print("/home/taylor/Research/plots/dtokpi/dspmass_breitwigner+dstd0bg_fit.eps")
canvas.Print("/home/taylor/Research/plots/dtokpi/dspmass_breitwigner+dstd0bg_fit.png")
