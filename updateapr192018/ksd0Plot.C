...

{
  ...
  TTree *t1=(TTree*) f1->Get("d0kstree"); //put tree name here
  ...
  RooPlot* frame1 = d0mass.frame(Bins(nBins),Title("D^{0} Mass (D^{0} -> K^{0}_{S}#pi^{0}) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  canvas->Print("./ksd0.pdf","pdf"); 
  canvas->Print("./ksd0.eps","eps");
