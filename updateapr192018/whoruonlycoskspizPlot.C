...
void whoruonlycoskspizPlot()

{
  
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/ksrecon.root"); //put file here
  TTree *t1=(TTree*) f1->Get("dsprecontree"); //put tree name here
  ...
  //----------------------------------------------------------
  double lb = coskspiz.getMin(); double rb=coskspiz.getMax();
  ...
  RooPlot* frame1 = coskspiz.frame(Bins(nBins),Title("Truth Matched cos(#theta_{K^{0}_{S}#pi^{0}}) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("cos(#theta_{K^{0}_{S}#pi^{0}})");
  ...
  TLegend *leg1 = new TLegend(0.4,0.6,0.7,0.8);
  //TLegend *leg1 = new TLegend(0.65,0.65, 0.87,0.85);
  leg1->SetFillColor(kWhite);
  leg1->SetLineColor(kWhite);
  leg1->SetTextSize(0.035);
  leg1->AddEntry(frame1->findObject("hd"),"Very Loose Truth Matched","l");
  //leg1->AddEntry(frame1->findObject("hd"),"Total","f");
  leg1->AddEntry(frame1->findObject("hd2"),"Loose Truth Matched","l");
  //leg1->AddEntry(frame1->findObject("hd3"),"Signal","f"); 
  // If you don't want filled histograms comment out the above 2 lines and uncomment the following 2 lines.
  leg1->AddEntry(frame1->findObject("hd3"),"Tight Truth Matched","l");
  //leg1->AddEntry(frame1->findObject("hd2"),"Background","f");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("./whoruonlycoskspiz.pdf","pdf"); 
  canvas->Print("./whoruonlycoskspiz.eps","eps");
