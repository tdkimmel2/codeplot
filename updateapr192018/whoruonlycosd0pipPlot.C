...

{
  ...
  RooPlot* frame1 = cosd0pip.frame(Bins(nBins),Title("Truth Matched cos(#theta_{D^{0}#pi^{+}}) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("cos(#theta_{D^{0}#pi^{+}})");
  ...
  canvas->Print("./whoruonlycosd0pip.pdf","pdf"); 
  canvas->Print("./whoruonlycosd0pip.eps","eps");
