...

{
  ...
  RooPlot* frame1 = cosd0pip.frame(Bins(nBins),Title("cos(#theta_{D^{0}#pi^{+}}) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  leg1->AddEntry(frame1->findObject("hd"),"No Cut","l");
  //leg1->AddEntry(frame1->findObject("hd"),"Total","f");
  leg1->AddEntry(frame1->findObject("hd2"),"cos(#theta_{D^{0}#pi^{+}}) > 0.5","l");
  ...
  leg1->AddEntry(frame1->findObject("hd3"),"cos(#theta_{D_{0}#pi^{+}}) > 0.8","l");
  //leg1->AddEntry(frame1->findObject("hd2"),"Background","f");
  //frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("./onlycosd0pip.pdf","pdf"); 
  canvas->Print("./onlycosd0pip.eps","eps");
