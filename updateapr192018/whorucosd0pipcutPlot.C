...

{
  ...
  hd2->SetLineWidth(2);
  t1->Draw("dspmass>>hd2","cosd0pip > 0.5 && whoru==1");

  TH1F *hd3 = new TH1F("hd3","hd3",nBins,lb,rb);
  hd3->SetLineColor(kRed);
  //hd3->SetFillColor(kRed);
  hd3->SetLineWidth(2);
  t1->Draw("dspmass>>hd3","cosd0pip > 0.8 && whoru==1");
  ...
  RooPlot* frame1 = dspmass.frame(Bins(nBins),Title("Very Loose Truth Matched D*^{+} Mass From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  leg1->AddEntry(frame1->findObject("hd"),"No Angle Cut","l");
  ...
  leg1->AddEntry(frame1->findObject("hd3"),"cos(#theta_{D^{0}#pi^{+}}) > 0.8","l");
  //leg1->AddEntry(frame1->findObject("hd2"),"Background","f");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("./whorucosd0pipcut.pdf","pdf"); 
  canvas->Print("./whorucosd0pipcut.eps","eps");
