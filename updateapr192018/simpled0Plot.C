...

{
  ...
  TTree *t1=(TTree*) f1->Get("d0tree"); //put tree name here
  ...
  RooPlot* frame1 = d0mass.frame(Bins(nBins),Title("D^{0} Mass (D^{0} -> K^{-}#pi^{+}) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  canvas->Print("./simpled0near.pdf","pdf"); 
  canvas->Print("./dimpled0near.eps","eps");
