...

{
  ...
  RooPlot* frame1 = dspmass.frame(Bins(nBins),Title("D*^{+} Mass (D*^{+}->#pi^{+}(D^{0} -> K^{-}#pi^{+})) From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  frame1->addTH1(hd3);
  ...
  canvas->Print("./simpledsp.pdf","pdf"); 
  canvas->Print("./simpledsp.eps","eps");
