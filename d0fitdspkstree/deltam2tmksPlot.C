...
void deltam2tmksPlot()

{
  
  int nBins = 100; 
  TFile *f1=new TFile("/home/taylor/Research/root/dsprecond0fit.root"); //put file here
  ...
  hd->SetLineWidth(2);
  t1->Draw("deltam2>>hd", "d0mass < 1.994 && d0mass > 1.706");
  ...
  RooPlot* frame1 = deltam2.frame(Bins(nBins),Title("#DeltaM^{2} With 5#sigma D^{0} Mass Window From MC Data")); //Edit the Title of the plot here
  frame1->addTH1(hd);
  //frame1->addTH1(hd3);
  ...
  frame1->GetXaxis()->SetTitle("#DeltaM^{2} (GeV^{2}/c^{4})");
  ...
  TLegend *leg1 = new TLegend(0.15, 0.65, 0.35, 0.85); //(x1, y1, x2, y2) of legend box location
  leg1->SetFillColor(kWhite);
  leg1->SetLineColor(kWhite);
  leg1->SetTextSize(0.035);
  leg1->AddEntry(frame1->findObject("hd"),"5#sigma D^{0} Mass Range Cut","l");
  ...
  leg1->AddEntry(frame1->findObject("hd2"),"Truth Matched, No D^{0} Mass Cut","l");
  frame1->addObject(leg1);

  frame1->Draw(); 
  
  canvas->Print("/home/taylor/Research/plots/d0fitdspkstree/deltam2tmks.pdf","pdf"); 
  canvas->Print("/home/taylor/Research/plots/d0fitdspkstree/deltam2tmks.eps","eps");
